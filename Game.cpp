...
{
	score = (lv - 140) / 50;
	if (score == 1) delay = 200;
	else if (score == 2) delay = 100;
	else if (score == 3) delay = 40;
	else if (score == 4) delay = 10;
	else if (score == 5) delay = 2;
}
...
void Game::createFruit()
{
	...
	{
		fruit_.x = rand() % ((SCREEN_WIDTH-60)/SIZE_SNAKE) ;
		fruit_.y = rand() % ((SCREEN_HEIGHT-50-40-20)/SIZE_SNAKE) ;
		is_create_fruit = false;
	}
	else if (eating() == true)
	{
		fruit_.x = 3+rand() % ((SCREEN_WIDTH - 60) / SIZE_SNAKE);
		fruit_.y = 3 + rand() % ((SCREEN_HEIGHT - 50 - 40-20) / SIZE_SNAKE);
...
bool Game::eating()
{
	if (snake_[0].x == fruit_.x * 20 && snake_[0].y == fruit_.y * 20) {
		realsizesnake++;
		SC += score;
		iswhite = false;
		return true;
...
void Game::draw(SDL_Renderer* renderer)
{
	...
	SDL_Texture* background = loadTexture("images/blacksky.jpg", renderer);
	SDL_Texture* sprite = loadTexture("images/snakebaby.jpg", renderer);
	SDL_Texture* wall = loadTexture("images/Wall.png", renderer);
	...
	{
		SDL_Delay(500);
		SDL_Texture* endgame = loadTexture("images/gameover.bmp", renderer);
...
void Game::audio()
{
	...
	gameover = Mix_LoadMUS("audio/game_over.mp3");
	eat = Mix_LoadMUS("audio/eat.mp3");
